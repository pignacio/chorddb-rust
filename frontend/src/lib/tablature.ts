...
export function expandAll(
	tablature: Tablature,
	fingerings: { [key: string]: Fingering }
): Tablature {
...
export function expandFingerings(bit: TabBit, fingerings: { [key: string]: Fingering }): TabBit[] {
	if (bit.type == 'chord') {
		const chord = bit.chord;
		const fingering: Fingering = fingerings[chord] ?? {
			frets: [],
			joined: 'UNKNOWN'
		};
...
class RenderLine {
	...
	addBit(bit: TabBit) {
		this.bits.push(bit);
		this.lastPosition =
			this.lastPosition == undefined ? bit.position : Math.min(this.lastPosition, bit.position);
	}
}
